...
void set_req_variables(unsigned int* servers_selected);
...
int main(int argc, char *argv[])
{
    ...
    printf("Allocating servers_selected\n");
    servers_selected = (unsigned int*)calloc(num_server, sizeof(unsigned int));
    printf("Allocating servers_selected - DONE\n");

    if (!servers_selected)
    {
        cleanup();
        error("Error: calloc servers_selected");
    }
    set_req_variables(servers_selected);
    ...
    printf("===========================================\n");
    // exit_connections();
    sleep(5); //time to allow all the requests to complete.
...
void set_req_variables(unsigned int* servers_selected)
{
    unsigned int i, k, l, server_id, flow_id = 0;
    ...
    for (i = 0; i < req_total_num; i++)
    {
        ...
        if (servers_selected!=NULL)
        {

		    for (l = 0; l < num_server; ++l)
		    {
		        /* code */
		        // printf("servers_selected%i\n", servers_selected[l]);
		        servers_selected[l]=0;
		    }
        ...
        {
            printf("Server selected is null\n");
            cleanup();
            exit(EXIT_FAILURE);
...
void *listen_connection(void *ptr)
{
    ...
    while (true)
    {
        ...
        // else
        // {
            ret_val = read_exact_until_vDup(node->sockfd, read_buf, flow.size, TG_MAX_READ, true, req_stop_time, flow_req_id[flow.id - 1], true, true);
        // }
        if (ret_val < 0)
        {
        ...
        //     if (verbose_mode)
        //     {
        //         printf("flowid: %i finished, new available len: %i\n", flow.id, node->list->available_len);    
        //     }
        ...
        // {
            ...
            if ((req_stop_time[flow_req_id[flow.id - 1]].tv_sec == 0) && (req_stop_time[flow_req_id[flow.id - 1]].tv_usec == 0)) // update only once
            {
    ...
    // if (node->list->len>=100)
    // {
    //     for (int i = 0; i < 1; ++i)
    //     {
    //         remove_conn_node_at_head(node->list);
    //     }
    // }
    // if (node!=NULL)
    // {
    ...
    //         print_conn_list(node->list);
    //         print_conn_node(node);
    //         if (remove_from_conn_list(node->list,node))
    //         {
    //             printf("remove node: success\n");
    //         }
    //         else
    //             printf("remove node: failure\n");   
...
void run_incast_request(unsigned int req_id)
{
    ...
    conn_id = 0;
    /*choose a primary server id*/
    int primary_server_id = rand() % num_server;
    while(req_server_flow_count[req_id][primary_server_id] == 0) //check if the server is actually used for this request
    {
        primary_server_id = rand() % num_server;
    }
    ...
    for (i = 0; i < num_server; i++)
    {
        ...
        if (verbose_mode)
        {
            printf("available len: %i\n", connection_lists[i].available_len);
            printf("num conns to search:%i\n", num_conn );
            print_conn_list(&connection_lists[i]);
        }
        ...
        if (incast_server_conn)
        {
            for (k = 0; k < num_conn; k++)
            {
                ...
                flow_reqs[conn_id].metadata.size = req_size[req_id]; /*duplicate request, instead of dividing*/
...
void print_statistic()
{
    ...
    for (i = 0; i < flow_total_num; i++)
    {
        ...
        if (fct_us > 0)
            flow_goodput_mbps = req_size[req_id] * 8 / fct_us;
        else
            flow_goodput_mbps = 0;

        /* flow size, FCT(us), DSCP, sending rate (Mbps), goodput (Mbps) */
        fprintf(fd, "%u %llu %u %u %u\n", req_size[req_id], fct_us, req_dscp[req_id], req_rate[req_id], flow_goodput_mbps);
    }
    fclose(fd);
